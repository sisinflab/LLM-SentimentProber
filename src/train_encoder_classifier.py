...
"""
Fine-tunes DeBERTa with two approaches:
...
Batch size is now included in the hyperparameter space, along with max_length.

Usage:
    python fine_tune.py --config_file configs/config.yaml
"""
...
RESULTS_FILE = "DeBERTa_best_model_results.tsv"
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Fine-tune DeBERTa for sentiment classification.")
    parser.add_argument(
        "--config_file",
        type=str,
        required=True,
...
def train_full_finetune(configs, train_dataset, test_dataset, num_labels, results, dataset_name):
    ...
    def objective(trial):
        ...
        max_length = configs.get('max_length', 256) #trial.suggest_int('max_length', 64, 256, step=32)
        learning_rate = trial.suggest_loguniform('learning_rate', 1e-5, 1e-3)
        num_epochs = trial.suggest_int('num_epochs', 2, 10)
        batch_size = configs.get('batch_size', 16) #trial.suggest_categorical('batch_size', [8, 16, 32])
...
def train_frozen_with_classifier(configs, texts_train, labels_train, texts_test, labels_test, results, dataset_name):
    """Train a frozen DeBERTa with a classifier attached, optimized with Optuna."""
    classifiers = {
        'SVM': SVC,
    ...
    def objective(trial):
        max_length = configs.get('max_length', 256) #trial.suggest_int('max_length', 64, 256, step=32)
        batch_size = configs.get('batch_size', 8) #trial.suggest_categorical('batch_size', [8, 16, 32])
